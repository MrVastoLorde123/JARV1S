...
function Get-ServerModelIds {
    ...
    try {
        $result = Invoke-RestMethod -Uri ($BaseUrl + "/v1/models") -Method Get -TimeoutSec 5
        if ($null -eq $result.data) { return @() }
        return @($result.data | ForEach-Object { if ($null -ne $_.id) { [string]$_.id } })
    }
    catch { return @() }
}
...
function Resolve-ServerModel {
    ...
    if ($modelIds -contains $RequestedModel) { return $RequestedModel }
...
function Test-HttpReady {
    param([Parameter(Mandatory)][string]$BaseUrl)
    try {
        $response = Invoke-WebRequest -Uri ($BaseUrl + "/v1/models") -Method Get -TimeoutSec 3 -UseBasicParsing
...
function Resolve-Model {
    param([string]$ExplicitPath, [Parameter(Mandatory)][string]$RepoRoot)
    if ($ExplicitPath) {
        if (-not (Test-Path -LiteralPath $ExplicitPath -PathType Leaf)) { Fail "Model file was not found: $ExplicitPath" }
        return (Resolve-Path -LiteralPath $ExplicitPath).Path
    }
    $candidateRoots = @((Join-Path $RepoRoot "models"), (Join-Path $env:USERPROFILE "models"))
    $models = @()
...
function Stop-StartedServer {
    param([System.Diagnostics.Process]$Process, [bool]$StartedByScript)
    if (-not $StartedByScript -or $null -eq $Process) { return }
    try {
...
try {
    ...
    if (-not $SkipGitCheck) {
        ...
        if ($LASTEXITCODE -ne 0) { Fail "Git is not available in this terminal." }
        Write-Host "Branch     : $branch"
        if ($branch -eq "main") { Write-Host "Warning: milestone development should use a feature branch." -ForegroundColor Yellow }
    }
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Database bootstrap failed. JARVIS will not be launched." }
    ...
    if ($alreadyListening) {
        ...
        if (-not (Test-HttpReady -BaseUrl $baseUrl)) { Fail "Port $Port is occupied, but the local OpenAI-compatible /v1/models endpoint is not healthy." }
        ...
        $arguments = @("--model", $modelPath, "--alias", $ModelAlias, "--host", $BindHost, "--port", $Port, "--ctx-size", $ContextSize)
        if ($AdditionalLlamaArgs.Count -gt 0) { $arguments += $AdditionalLlamaArgs }
        Write-Host "Starting llama-server..." -ForegroundColor Green
        Write-Host "Logs       : $stdoutLog"
        $serverProcess = Start-Process -FilePath $serverPath -ArgumentList $arguments -WorkingDirectory $serverDirectory -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog -PassThru
    ...
    if ($SessionId) { $env:JARVIS_SESSION_ID = $SessionId } else { Remove-Item Env:JARVIS_SESSION_ID -ErrorAction SilentlyContinue }
    ...
    if ($SkipTests) {
        ...
        if ($LASTEXITCODE -ne 0) { Fail "AI provider regression tests failed. JARVIS will not be launched." }
        Write-Host "Running core suite..."
        python -m unittest discover -s src\core -p "test*.py"
        if ($LASTEXITCODE -ne 0) { Fail "Core tests failed. JARVIS will not be launched." }
    ...
    if ($jarvisExitCode -ne 0) { Write-Host "JARVIS exited with code $jarvisExitCode." -ForegroundColor Yellow }
}
finally {
