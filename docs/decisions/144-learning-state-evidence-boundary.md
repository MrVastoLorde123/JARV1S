...
The regression run initially exposed one defect in `test_invalid_integrity_fails_closed`: internally tampered application lineage within an otherwise `VALID` integrity artifact was accepted as `RECORDED`. The production boundary was corrected to require preserved application and decision lineage to remain internally consistent before recording state evidence. The corrected focused and full M23.132–M23.124 regression block then passed successfully.
