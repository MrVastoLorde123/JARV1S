...
- Requires interpretation identity to be distinct from the request identity and preserved upstream source validation identity.
- Requires request, direct validation-event lineage, read, and consumption-request lineage to remain internally consistent.
- Preserves the upstream `source_validation_id` carried by M23.139 while treating its direct validation-event lineage identity separately.
...
## Verification
Focused verification: **15/15 M23.140 tests passed**.

The focused suite validates request status, identity separation, lineage fail-closed behavior, recursive immutability, deterministic structural interpretation, provenance preservation, source non-mutation, and absence of truth/correctness/certainty/usefulness/authority powers.
