...
A worker receives an explicit immutable projection of context selected by its
assignment. The projection is not the global JARVIS WorkingContext and does
not confer authority, permissions, credentials, or unrestricted knowledge.
"""
...
class WorkerContextProjector:
    ...
    def project(
        ...
        return WorkerContext(
            ...
            inputs=inputs,
            facts=tuple(item for item in global_context.context_package.items if "facts" in assignment.input_scope),
            observations=tuple(global_context.observations if "observations" in assignment.input_scope else ()),
            metadata={
                "context_boundary": "m9.3",
                "source_global_context": False,
