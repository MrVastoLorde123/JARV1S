...
class FakeLLMHandler(BaseHTTPRequestHandler):
    ...
    models_payload = {
        "object": "list",
        "data": [
            {"id": "qwen3-4b-local", "object": "model"},
            {"id": "granite-8b", "object": "model"},
        ],
    }
    last_request = None
    ...
    def do_GET(self):
        if self.path != "/v1/models":
            ...
            self.end_headers()
            return
        response = json.dumps(self.models_payload).encode("utf-8")
    ...
    def do_POST(self):
        ...
        FakeLLMHandler.last_request = json.loads(body.decode("utf-8"))
...
class LocalProviderTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.server = HTTPServer(("127.0.0.1", 0), FakeLLMHandler)
    ...
    def test_list_models_observes_openai_compatible_model_ids(self):
        self.assertEqual(
            self.provider.list_models(),
            ("qwen3-4b-local", "granite-8b"),
        )

    def test_generate_returns_ai_response(self):
        ai_request = AIRequest(task="Explain JARVIS.", context=None)
        ...
        self.assertEqual(response.model, "qwen3-4b-local")
        self.assertEqual(response.content, "Fake local response.")

    def test_usage_is_parsed(self):
        ai_request = AIRequest(task="Test usage.", context=None)
        response = self.provider.generate(ai_request)
        self.assertIsNotNone(response.usage)
        self.assertEqual(response.usage.total_tokens, 30)

    def test_request_is_sent_to_server(self):
        ai_request = AIRequest(task="Explain Modbus.", context=None)
        self.provider.generate(ai_request)
        sent = FakeLLMHandler.last_request
        self.assertEqual(sent["model"], "qwen3-4b-local")
        self.assertEqual(sent["stream"], False)
        self.assertEqual(sent["messages"][-1]["content"], "Explain Modbus.")
        ...
        context = ContextPackage(
            ...
            items=(
                ContextItem(
                    ...
                    provenance={"memory_key": "pcvue_skill"},
                ),
            ),
            instructions=("Treat memory as evidence.",),
            metadata={"builder_version": "1.0"},
        )
        ai_request = AIRequest(task="What do you know?", context=context)
        self.provider.generate(ai_request)
        system_message = FakeLLMHandler.last_request["messages"][0]["content"]
        self.assertIn("JARVIS INSTRUCTIONS", system_message)
        self.assertIn("Treat memory as evidence.", system_message)
        self.assertIn("User is learning PCVUE.", system_message)

    def test_provider_does_not_require_openai_sdk(self):
        self.assertFalse(hasattr(self.provider, "openai"))
