...
class RoleFitnessProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.required_dimensions:
            raise ValueError("role fitness requires dimension weights")
        if not 0.0 <= self.minimum_overall_score <= 1.0:
            raise ValueError("minimum_overall_score must be between 0 and 1")
        if any(weight <= 0.0 for weight in self.required_dimensions.values()):
            raise ValueError("role dimension weights must be positive")
...
def evaluate_role_fitness(
    ...
    for dimension, minimum in profile.required_dimensions.items():
        ...
        weighted_values.append(actual * minimum)
        if actual < minimum:
            unmet.append(dimension)

    weight_total = sum(profile.required_dimensions.values())
    weighted_score = sum(weighted_values) / weight_total if weight_total else 0.0
    infrastructure_ok = (
        profile.allow_infrastructure_failures
        or report.infrastructure_failures == 0
    )
    suitable = (
