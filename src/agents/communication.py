...
class AgentCommunication:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.kind, AgentMessageKind): raise TypeError("kind must be an AgentMessageKind")
        object.__setattr__(self, "summary", _text(self.summary, "summary"))
        if not isinstance(self.urgency, AgentUrgency): raise TypeError("urgency must be an AgentUrgency")
        if self.question is not None: object.__setattr__(self, "question", _text(self.question, "question"))
        if len(self.needs) > _MAX_ITEMS: raise ValueError(f"needs exceeds {_MAX_ITEMS} items")
        if any(not isinstance(need, AgentNeed) for need in self.needs): raise TypeError("needs must contain AgentNeed values")
        object.__setattr__(self, "evidence", _freeze_mapping(self.evidence, "evidence"))
        if self.requested_action is not None: object.__setattr__(self, "requested_action", _text(self.requested_action, "requested_action"))
        if not isinstance(self.blocking, bool): raise TypeError("blocking must be a bool")
        object.__setattr__(self, "message_id", _text(self.message_id, "message_id"))
        if self.kind is AgentMessageKind.QUESTION and self.question is None: raise ValueError("QUESTION messages require question")
        if self.kind is AgentMessageKind.CAPABILITY_REQUEST and not self.needs: raise ValueError("CAPABILITY_REQUEST messages require at least one need")
        if self.kind is AgentMessageKind.TOOL_REQUEST and not self.needs: raise ValueError("TOOL_REQUEST messages require at least one need")
        if self.kind in {AgentMessageKind.BLOCKER, AgentMessageKind.ESCALATION} and not self.blocking: raise ValueError(f"{self.kind.value} messages must set blocking=True")
    @property
...
class AgentDirective:
    ...
    reason: str = ""
    def __post_init__(self) -> None:
        if not isinstance(self.kind, AgentDirectiveKind): raise TypeError("kind must be an AgentDirectiveKind")
        object.__setattr__(self, "summary", _text(self.summary, "summary"))
        ...
        if self.answer is not None: object.__setattr__(self, "answer", _text(self.answer, "answer"))
        if not isinstance(self.reason, str): raise TypeError("reason must be a string")
        if self.kind is AgentDirectiveKind.ANSWER and self.answer is None: raise ValueError("ANSWER directives require answer")
        if self.kind in {AgentDirectiveKind.GRANT, AgentDirectiveKind.PARTIAL_GRANT} and not self.granted_needs: raise ValueError(f"{self.kind.value} directives require granted_needs")
        if self.kind is AgentDirectiveKind.DENY and not self.denied_needs: raise ValueError("DENY directives require denied_needs")
...
class AgentCommunicationGuide:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.protocol_version, str) or not self.protocol_version.strip(): raise ValueError("protocol_version must be a non-empty string")
