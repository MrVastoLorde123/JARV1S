...
from src.safe_modification import ExecutionHandoffStatus, SafeModificationExecution, SafeModificationValidationError
...
def build_verified_execution() -> SafeModificationExecution:
    ...
    return SafeModificationExecution(
        execution_id="execution-1", verification=verification, status=ExecutionHandoffStatus.READY,
        execution_scope=("src/example.py",), preconditions=("working tree is clean",),
    )
...
class SafeModificationTests(unittest.TestCase):
    ...
    def test_failed_verification_cannot_be_ready(self):
        ...
        with self.assertRaises(SafeModificationValidationError):
            SafeModificationExecution(execution_id="execution-2", verification=failed,
                                      status=ExecutionHandoffStatus.READY)

    def test_default_state_is_not_executed(self):
        execution = SafeModificationExecution(execution_id="execution-3",
    ...
    def test_blocked_state_can_be_selected_explicitly(self):
    ...
    def test_metadata_is_frozen(self):
        execution = SafeModificationExecution(
            execution_id="execution-5", verification=build_verified_execution().verification,
    ...
    def test_constraints_do_not_create_permission(self):
        self.assertNotIn("permission_granted", build_verified_execution().to_dict())
