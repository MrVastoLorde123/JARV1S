...
class PersistentMemoryRepository:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.execute("""CREATE TABLE IF NOT EXISTS persistent_memory_records (
            ...
            connection.execute("""CREATE TABLE IF NOT EXISTS persistent_memory_provenance (
    ...
    def persist(self, record: PersistentMemoryRecord, provenance: ProvenanceChain) -> bool:
        ...
        with self._connect() as connection:
            ...
            if existing is not None:
                existing_payload = {
                    "kind": existing[0], "subject_id": existing[1], "content": existing[2],
                    "confidence": existing[3], "importance": existing[4], "status": existing[5],
                    "created_at": existing[6], "updated_at": existing[7], "supersedes_memory_id": existing[8],
                    "provenance_ids": tuple(json.loads(existing[9])), "tags": tuple(json.loads(existing[10])),
                    "metadata": json.loads(existing[11]),
                }
                incoming_payload = {
                    "kind": record.kind.value, "subject_id": record.subject_id, "content": record.content,
                    "confidence": float(record.confidence), "importance": float(record.importance),
                    "status": record.status.value, "created_at": record.created_at, "updated_at": record.updated_at,
                    "supersedes_memory_id": record.supersedes_memory_id, "provenance_ids": tuple(record.provenance_ids),
                    "tags": tuple(record.tags), "metadata": incoming_metadata,
                }
            ...
            for reference in provenance.refs:
                ...
                incoming_ref = (
                    reference.source_kind.value, reference.source_id, reference.summary,
                    reference.observed_at, reference.confidence,
                )
                if existing_ref is not None and tuple(existing_ref) != incoming_ref:
                    raise ValueError(f"provenance id already exists with different content: {reference.provenance_id}")
            ...
            connection.execute(
                ...
                (
                    record.memory_id, record.subject_id, record.kind.value, record.content,
                    float(record.confidence), float(record.importance), record.status.value,
                    record.created_at, record.updated_at, record.supersedes_memory_id,
                    json.dumps(record.provenance_ids), json.dumps(record.tags),
    ...
    def list_records(self, *, kind: PersistentMemoryKind | None = None, subject_id: str | None = None, status: MemoryLifecycle | None = None) -> tuple[PersistentMemoryRecord, ...]:
        clauses: list[str] = []
        values: list[object] = []
        if kind is not None:
        ...
        if subject_id is not None:
            ...
            clauses.append("subject_id = ?"); values.append(subject_id)
        if status is not None:
            if not isinstance(status, MemoryLifecycle):
                raise TypeError("status must be a MemoryLifecycle")
            clauses.append("status = ?"); values.append(status.value)
        where = " WHERE " + " AND ".join(clauses) if clauses else ""
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT memory_id, subject_id, kind, content, confidence, importance, status, created_at, updated_at, supersedes_memory_id, provenance_json, tags_json, metadata_json FROM persistent_memory_records" + where + " ORDER BY created_at, memory_id",
                tuple(values),
            ).fetchall()
    ...
    def _record_from_row(row: tuple[Any, ...]) -> PersistentMemoryRecord:
        return PersistentMemoryRecord(
            memory_id=row[0], subject_id=row[1], kind=PersistentMemoryKind(row[2]), content=row[3],
            confidence=float(row[4]), importance=float(row[5]), status=MemoryLifecycle(row[6]),
            created_at=row[7], updated_at=row[8], supersedes_memory_id=row[9],
            provenance_ids=tuple(json.loads(row[10])), tags=tuple(json.loads(row[11])), metadata=json.loads(row[12]),
        )
    ...
    def provenance_for(self, memory_id: str) -> ProvenanceChain:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT provenance_id, source_kind, source_id, summary, observed_at, confidence FROM persistent_memory_provenance WHERE provenance_id IN (" + placeholders + ")",
        ...
        refs = tuple(
            ProvenanceRef(
                ...
                source_id=row[2], summary=row[3], observed_at=row[4], confidence=row[5],
            )
...
class PersistentIntelligenceSystem:
    ...
    def remember(self, record: PersistentMemoryRecord, provenance: ProvenanceChain) -> bool:
        return self._repository.persist(record, provenance)

    def recall(self, *, kind: PersistentMemoryKind | None = None, subject_id: str | None = None, status: MemoryLifecycle | None = None) -> tuple[PersistentMemoryRecord, ...]:
    ...
    def consolidate(self, episodes: tuple[EpisodicMemory, ...]) -> tuple[ConsolidationProposal, ...]:
        return propose_semantic_consolidation(episodes)

    def project_personal_model(self, subject_id: str, entries: tuple[PersonalModelEntry, ...], *, model_version: int = 1) -> PersonalModel:
    ...
    def summary(self) -> Mapping[str, object]:
        ...
        return {
            "coverage": coverage.to_context(), "repository_path": str(self._repository.database_path),
            "truth_established": False, "authority_granted": False, "execution_requested": False,
            "capability_invocation_performed": False, "provider_selected": False,
        }

    def to_context(self) -> dict[str, object]:
        return {"summary": dict(self.summary()), "coverage": self.coverage().to_context(), "authority_granted": False, "execution_requested": False}


__all__ = ["PersistentIntelligenceCoverage", "PersistentIntelligenceSystem", "PersistentMemoryRepository"]
