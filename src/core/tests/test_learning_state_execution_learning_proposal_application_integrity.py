...
class M23_131LearningProposalApplicationIntegrityTests(unittest.TestCase):
    ...
    def test_application_identity_and_provenance_are_preserved(self):
        ...
        for field in (
            ...
            "eligibility_purpose", "proposer_id", "proposal_purpose", "decision_maker_id", "decision_purpose", "applier_id", "application_purpose", "application_status",
        ):
            self.assertEqual(getattr(result, field), getattr(application, "integrity_id" if field == "source_integrity_id" else field) if field != "source_integrity_id" else application.integrity_id)
    ...
    def test_nested_evidence_and_lineage_are_recursively_frozen(self):
        result = LearningStateExecutionLearningProposalApplicationIntegrityService().validate(self._make_application(), integrity_id="application-integrity-131")
        self.assertIsInstance(result.application_evidence, tuple.__mro__[1] if False else object)
