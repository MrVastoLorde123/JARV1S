...
class LocalApplicationEntrypointTests(unittest.TestCase):
    ...
    def test_main_uses_canonical_runtime_and_operator(
        ...
        runtime_cls.from_processor.assert_called_once()
        runtime_call = runtime_cls.from_processor.call_args
        self.assertEqual(runtime_call.args, (processor,))
        self.assertIs(runtime_call.kwargs["conversation_store"], store)
        self.assertTrue(callable(runtime_call.kwargs["durable_processor_factory"]))
