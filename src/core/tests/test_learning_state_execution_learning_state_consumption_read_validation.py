...
from src.core.tests.test_learning_state_execution_learning_state_durable_read_consumption import (
    M23_137LearningStateDurableReadConsumptionTests,
)
...
class M23_138LearningStateConsumptionReadValidationTests(unittest.TestCase):
    ...
    def test_non_hex_fingerprint_is_rejected(self):
        read = self._make_read()
        object.__setattr__(read, "read_fingerprint", "z" * 64)
        result = self._validate(read)
        self.assertIs(result.status, LearningStateExecutionLearningStateConsumptionReadValidationStatus.REJECTED)
        self.assertIn("read fingerprint mismatch", result.reasons)
        self.assertIn("read fingerprint is not SHA-256", result.reasons)

    def test_tampered_scope_state_key_is_rejected(self):
        read = self._make_read()
        object.__setattr__(read, "requested_scope", {"state_key": "other.state"})
        result = self._validate(read)
        self.assertIs(result.status, LearningStateExecutionLearningStateConsumptionReadValidationStatus.REJECTED)
        self.assertIn("requested scope state key mismatch", result.reasons)

    def test_tampered_field_bounded_payload_is_rejected(self):
        read = self._make_read(scope={"state_key": "demo.state", "fields": ["threshold"]})
        object.__setattr__(read, "read_payload", {"mode": "safe"})
        result = self._validate(read)
        self.assertIs(result.status, LearningStateExecutionLearningStateConsumptionReadValidationStatus.REJECTED)
        self.assertIn("field-bounded read payload does not match requested fields", result.reasons)
    ...
    def test_caller_reasons_and_lineage_are_preserved_and_frozen(self):
        result = self._validate(
            ...
            lineage={"validation_id": "read-validation-138", "read_id": "read-137", "chain": {"step": 1}},
        )
