...
class M23_80ApplicationLearningSignalV3Tests(unittest.TestCase):
    ...
    def test_signal_status_mismatch_is_rejected(self):
        s=self._evaluation(E.SUCCESS_EVALUATION); k=dict(s.__dict__); k["signal_id"]="signal-new"; k["signal_status"]=L.NEGATIVE_SIGNAL
    ...
    def test_invalid_integrity_is_rejected(self):
        s=self._evaluation(E.SUCCESS_EVALUATION); k=dict(s.__dict__); k.update(signal_id="signal-new",signal_status=L.POSITIVE_SIGNAL,integrity_status=I.INVALID)
        with self.assertRaises(ValueError): EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningAdaptationApplicationLearningSignalV3(**k)
    def test_non_mapping_lineage_is_rejected(self):
        s=self._evaluation(E.SUCCESS_EVALUATION); k=dict(s.__dict__); k.update(signal_id="signal-new",signal_status=L.POSITIVE_SIGNAL,lineage=[])
