"""M23.71: integrity validation of v3 adaptation-execution learning signals."""
...
class EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Error(RuntimeError):
    """Raised when v3 learning-signal integrity evidence cannot be formed safely."""


class EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Status(str, Enum):
    VALID = "VALID"
    INVALID = "INVALID"
...
def _learning_signal_fingerprint(
    signal: EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalV3,
) -> str:
    payload = {
        ...
        "proposal_fingerprint": signal.proposal_fingerprint,
...
class EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3:
    """Immutable integrity evidence over one v3 learning signal."""
    ...
    status: EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Status
    ...
    def __post_init__(self) -> None:
        for name in (
            "integrity_id", "signal_id", "evaluation_id", "feedback_id", "classification_id",
            "source_integrity_id", "execution_id", "handoff_id", "authorization_id", "validation_id",
            "proposal_id", "eligibility_id", "source_signal_id", "outcome_id", "preparation_id",
        ...
        if not isinstance(self.status, EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Status):
            raise TypeError("status must be a learning-signal integrity v3 status")
        if self.status == EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Status.VALID:
...
class EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Service:
    """Verify one exact v3 learning signal and emit inert integrity evidence."""
    ...
    def verify(
        ...
        *,
        ...
        if type(signal) is not EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalV3:
            raise TypeError(
                "signal must be EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalV3"
            )
        ...
        return EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3(
            ...
            executor_id=signal.executor_id,
            failure_reason=signal.failure_reason,
            status=EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Status.VALID,
            integrity_evaluation_id=signal.evaluation_id,
            reasons=reasons if reasons is not None else {"status": "learning signal v3 structurally validated and fingerprinted"},
...
__all__ = [
    "EnvironmentWorldModelRollbackRepairRetryAdaptationExecutionLearningSignalIntegrityV3Error",
