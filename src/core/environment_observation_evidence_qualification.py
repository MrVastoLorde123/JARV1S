...
class EnvironmentObservationEvidenceQualificationService:
    ...
    def qualify_aggregate(
        ...
        *,
        ...
        normalized_consistency = tuple(consistency)
        self._validate_aggregate_bundle(aggregate, normalized_validities, normalized_consistency, provenance)

        expected_pairs = len(aggregate.observation_ids) * (len(aggregate.observation_ids) - 1) // 2
        consistency_states = tuple(item.consistency for item in normalized_consistency)
        if len(normalized_consistency) < expected_pairs:
            qualification = EvidenceQualification.INSUFFICIENT
            reasons = ("consistency evidence is incomplete",)
        elif any(item.consistency is ObservationConsistency.CONFLICTING for item in normalized_consistency):
            qualification = EvidenceQualification.CONFLICTING
            reasons = ("one or more supplied observation comparisons are CONFLICTING",)
        elif any(item.freshness is not ObservationFreshness.CURRENT for item in normalized_validities):
        ...
        return EnvironmentObservationEvidenceQualification(
            ...
            validity=tuple(item.freshness for item in normalized_validities),
    ...
    def _validate_aggregate_bundle(
        ...
        expected_ids = set(aggregate.observation_ids)
        pair_keys = {
            (item.left_observation_id, item.right_observation_id)
            for item in consistency
        }
        if len(pair_keys) != len(consistency):
            raise EnvironmentObservationEvidenceQualificationError("duplicate consistency evidence pair")
        if any(
            left not in expected_ids or right not in expected_ids or left == right
            for left, right in pair_keys
        ):
            raise EnvironmentObservationEvidenceQualificationError("consistency evidence contains an invalid source pair")
        for item in consistency:
