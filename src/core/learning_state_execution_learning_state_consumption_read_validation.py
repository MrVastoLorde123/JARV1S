...
class LearningStateExecutionLearningStateConsumptionReadValidationService:
    ...
    def validate(
        ...
        *,
        ...
        scope = read.requested_scope
        if not isinstance(scope, Mapping):
            checks.append("requested scope must be a mapping")
            fields = None
        else:
            scope_state_key = scope.get("state_key", read.state_key)
            if scope_state_key != read.state_key:
                checks.append("requested scope state key mismatch")
            fields = scope.get("fields") if "fields" in scope else None
            if "fields" in scope:
        ...
        if fields is not None:
            if not isinstance(read.read_payload, Mapping):
                checks.append("field-bounded read payload must be a mapping")
            else:
                payload_keys = {str(key) for key in read.read_payload.keys()}
                expected_keys = set(fields)
                if payload_keys != expected_keys:
                    checks.append("field-bounded read payload does not match requested fields")
        ...
        if not _is_sha256(read.read_fingerprint):
            checks.append("read fingerprint is not SHA-256")
        if not _is_sha256(read.computed_read_fingerprint):
