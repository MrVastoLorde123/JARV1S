"""Feedback boundary after the M22.51 future execution result integrity artifact.

M22.52 converts exactly one M22.51 result-integrity artifact into immutable
feedback evidence. The historical M22.44 namespace remains untouched.
Feedback is observational only and cannot authorize, execute, retry, revoke,
mutate memory, or establish adaptation truth.
"""
...
from .learning_write_adaptation_evaluation_execution_feedback_result_integrity_feedback_evaluation_decision_proposal_admission_preparation_execution_result_integrity import (
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackError(ValueError):
    """Raised when the dedicated M22.52 feedback contract is invalid."""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedback:
    """Immutable feedback evidence derived from one exact M22.51 artifact."""
    ...
    def __post_init__(self) -> None:
        for field_name, value in (
            ("feedback_id", self.feedback_id), ("integrity_id", self.integrity_id),
        ...
        if not isinstance(self.kind, LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackKind):
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackError(
    ...
    def to_context(self) -> dict[str, object]:
        return {
            ...
            "learning_write_adaptation_evaluation_execution_result_integrity_feedback_kind": self.kind.value,
            "payload": dict(self.payload),
            "provenance": dict(self.provenance),
            "learning_write_adaptation_evaluation_execution_result_integrity_feedback_reason": self.reason,
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackService:
    """Convert one M22.51 result-integrity artifact into inert feedback evidence."""

    def from_integrity(self, integrity):
        if not isinstance(
            integrity,
            LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionPreparationExecutionResultIntegrity,
        ):
            raise TypeError(
                "integrity must be a LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionPreparationExecutionResultIntegrity"
            )

        if integrity.status is LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionPreparationExecutionResultIntegrityStatus.SUCCEEDED:
            ...
            payload = {
                ...
                "execution_result": integrity.execution_result,
        ...
        elif integrity.status is LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionPreparationExecutionResultIntegrityStatus.FAILED:
            kind = LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackKind.INTEGRITY_FAILURE
            payload = {"integrity_status": integrity.status.value, "reason": integrity.reason}
            reason = "failed result-integrity evidence provides observable future adaptation execution feedback"
        else:
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackError("unsupported result-integrity status")

        provenance = {
            "source": "learning_write_adaptation_evaluation_execution_feedback_result_integrity_feedback_evaluation_decision_proposal_admission_preparation_execution_result_integrity",
            ...
            "source_feedback_id": integrity.feedback_id,
        ...
        feedback_id = self._feedback_id(integrity, kind, payload)
    ...
    @staticmethod
    def _feedback_id(integrity, kind, payload):
        ...
        return "adaptation-evaluation-execution-feedback-result-integrity-feedback-preparation-execution-result-integrity-feedback-" + hashlib.sha256(serialized).hexdigest()[:24]
