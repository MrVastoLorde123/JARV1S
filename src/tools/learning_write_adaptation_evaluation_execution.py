...
M22.34 consumes only an exact M22.33 preparation artifact. It creates an
execution request and invokes a replaceable applier. Execution results are
immutable observations; execution does not grant authorization, retry,
revocation, or memory mutation authority.
"""
    ...
    def __post_init__(self) -> None:
        for field_name, value in (
            ...
            if not isinstance(value, str) or not value.strip():
                raise LearningWriteAdaptationEvaluationExecutionError(
...
class LearningWriteAdaptationEvaluationExecutionResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.status, LearningWriteAdaptationEvaluationExecutionStatus):
            raise LearningWriteAdaptationEvaluationExecutionError(
                "status must be a LearningWriteAdaptationEvaluationExecutionStatus member"
            )
...
class LearningWriteAdaptationEvaluationExecutionService:
    ...
    def execute(
        self,
        preparation: LearningWriteAdaptationEvaluationExecutionPreparation,
        ...
        if not isinstance(preparation, LearningWriteAdaptationEvaluationExecutionPreparation):
            raise TypeError(
                "preparation must be a LearningWriteAdaptationEvaluationExecutionPreparation"
            )
        if preparation.execution_authorized:
            raise LearningWriteAdaptationEvaluationExecutionError(
                "preparation cannot carry execution authorization"
            )
        if preparation.execution_started:
            raise LearningWriteAdaptationEvaluationExecutionError(
                "preparation must not already be marked as started"
            )
        if preparation.retry_requested or preparation.revocation_requested:
            raise LearningWriteAdaptationEvaluationExecutionError(
                "preparation cannot request retry or revocation"
            )
        if preparation.memory_mutation_allowed:
            raise LearningWriteAdaptationEvaluationExecutionError(
                "preparation cannot permit memory mutation"
            )
    ...
    def _execution_id(
        ...
        return f"adaptation-evaluation-execution-{hashlib.sha256(payload).hexdigest()[:24]}"
