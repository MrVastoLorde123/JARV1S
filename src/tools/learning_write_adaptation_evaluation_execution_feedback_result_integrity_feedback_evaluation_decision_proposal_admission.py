"""Admission boundary after M22.47 evaluation-decision proposal.

M22.48 admits or rejects exactly one immutable M22.47 proposal. Admission is
policy evidence, not authorization, execution permission, retry authority,
revocation, memory mutation, or proof of adaptation truth.
"""
...
from .learning_write_adaptation_evaluation_execution_feedback_result_integrity_feedback_evaluation_decision_proposal import (
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionError(ValueError):
    """Raised when the M22.48 admission contract is invalid."""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionStatus(str, Enum):
    """Normalized admission status for one M22.47 proposal."""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionContext:
    """Immutable context supplied to an M22.48 admission provider."""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmission:
    """Immutable admission result for exactly one M22.47 proposal."""
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            "outcome_id", "execution_id", "preparation_id", "source_admission_id",
            ...
            "source_execution_id", "domain", "source_policy_id", "policy_id", "reason",
        ):
        ...
        if self.status is LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionStatus.ADMITTED and not self.proposal:
        ...
        if self.status is LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionStatus.ADMITTED and not self.evidence:
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionError("admitted proposal evidence cannot be empty")
        if self.status is LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionStatus.ADMITTED and not self.provenance:
        ...
        if self.execution_authorized or self.authorization_granted or self.execution_requested or self.retry_requested or self.revocation_requested or self.memory_mutation_allowed or self.authority_granted:
...
class DeterministicLearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionProvider:
    """Deterministic baseline provider for M22.48 proposal admission."""

    _POLICY_ID = "adaptation-evaluation-execution-feedback-result-integrity-proposal-admission-baseline-v1"
    ...
    def admit(self, context: LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionContext) -> LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmission:
        ...
        if proposal.kind is not LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalKind.ACCEPTED_EVALUATION:
    ...
    def _admission_id(proposal: Any, status: LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionStatus, reason: str) -> str:
        ...
        return "adaptation-evaluation-execution-feedback-result-integrity-proposal-admission-" + hashlib.sha256(serialized).hexdigest()[:24]
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionService:
    ...
    def admit(self, context: LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmissionContext) -> LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmission:
        ...
        if not isinstance(admission, LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalAdmission):
            raise TypeError("provider must return an M22.48 admission artifact")
        proposal = context.proposal
        expected = (
            ...
            ("outcome", admission.outcome_id, proposal.outcome_id),
