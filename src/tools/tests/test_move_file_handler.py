...
from src.tools.handlers.move_file import DEFAULT_MAX_FILE_SIZE_BYTES, MoveFileHandler
from src.tools.models import RiskLevel, ToolRequest


class MoveFileHandlerTestCase(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self._tmp.cleanup)
        self.base_dir = Path(self._tmp.name)
        self.handler = MoveFileHandler(self.base_dir)
...
class TestDefinition(MoveFileHandlerTestCase):
    ...
    def test_constructor_rejects_missing_base_dir(self) -> None:
        with self.assertRaises(ValueError):
            MoveFileHandler(self.base_dir / "does-not-exist")

    def test_constructor_rejects_invalid_max_file_size(self) -> None:
        with self.assertRaises(ValueError):
            MoveFileHandler(self.base_dir, max_file_size_bytes=0)


class TestSuccessfulMove(MoveFileHandlerTestCase):
    def test_moves_file_within_workspace(self) -> None:
        ...
        source_path = self.base_dir / "source.txt"
    ...
    def test_renames_file(self) -> None:
        self.write("old_name.txt", "content")
        request = ToolRequest(
            tool_name="move_file",
            arguments={"source_path": "old_name.txt", "destination_path": "new_name.txt"},
        )

        result = self.handler.execute(request)

        self.assertTrue(result.success)
        old_path = self.base_dir / "old_name.txt"
        new_path = self.base_dir / "new_name.txt"
        self.assertFalse(old_path.exists())
        self.assertTrue(new_path.exists())

    def test_moves_file_to_nested_directory(self) -> None:
...
class TestInvalidArguments(MoveFileHandlerTestCase):
    def test_missing_source_path_argument(self) -> None:
        result = self.handler.execute(
            ToolRequest(tool_name="move_file", arguments={"destination_path": "dest.txt"})
        )
    ...
    def test_missing_destination_path_argument(self) -> None:
        result = self.handler.execute(
            ToolRequest(tool_name="move_file", arguments={"source_path": "source.txt"})
        )
        self.assertFalse(result.success)
        self.assertEqual(result.error.code, "invalid_argument")
...
class TestFileNotFound(MoveFileHandlerTestCase):
...
class TestFileTooLarge(MoveFileHandlerTestCase):
...
class TestDestinationExists(MoveFileHandlerTestCase):
...
class TestParentNotFound(MoveFileHandlerTestCase):
...
class TestNotAFile(MoveFileHandlerTestCase):
...
class TestWorkspaceBoundary(MoveFileHandlerTestCase):
