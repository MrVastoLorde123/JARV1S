"""Evaluation boundary after M22.44 result-integrity feedback.

M22.45 converts exactly one M22.44 result-integrity feedback artifact into
immutable evaluation evidence. Evaluation preserves lineage and observed
evidence, but does not establish adaptation truth or grant authority.
"""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError(ValueError):
    """Raised when the M22.45 evaluation contract is invalid."""
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluation:
    """Immutable evaluation evidence derived from one exact M22.44 feedback artifact."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.signal, LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationSignalKind):
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError("invalid signal")
        if isinstance(self.confidence, bool) or not isinstance(self.confidence, (int, float)):
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError("confidence must be numeric")
        if not 0.0 <= float(self.confidence) <= 1.0:
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError(
                "confidence must be between 0.0 and 1.0"
            )
        if not isinstance(self.evidence, Mapping):
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError("evidence must be a mapping")
        if not isinstance(self.provenance, Mapping):
            raise LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationError("provenance must be a mapping")
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationService:
    """Evaluate exactly one M22.44 result-integrity feedback artifact."""
    ...
    def _evaluation_id(feedback: Any, signal: Any, evidence: Mapping[str, Any]) -> str:
        ...
        return (
            "adaptation-evaluation-execution-feedback-result-integrity-evaluation-"
