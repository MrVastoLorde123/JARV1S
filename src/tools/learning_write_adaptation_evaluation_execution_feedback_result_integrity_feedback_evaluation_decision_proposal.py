"""Proposal boundary after M22.46 result-integrity feedback evaluation decision.

M22.47 converts an ACCEPT decision into immutable proposal evidence. DEFER and
REJECT produce no proposal. A proposal is not authorization or execution.
"""
...
from .learning_write_adaptation_evaluation_execution_feedback_result_integrity_feedback_evaluation_decision import (
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposal:
    """Immutable proposal derived from exactly one accepted M22.46 decision."""
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            "preparation_id", "admission_id", "source_proposal_id", "decision_source_evaluation_id",
            "evaluation_id_from_feedback", "source_feedback_id", "candidate_id", "source_candidate_id",
            "execution_source_id", "source_execution_id", "source_admission_id", "proposal_source_id",
            "domain", "source_policy_id", "policy_id", "reason",
        ):
        ...
        if not isinstance(self.action, LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationAction):
        ...
        if self.action is not LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationAction.ACCEPT:
...
class LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalService:
    """Create a non-authorizing proposal from exactly one M22.46 decision."""
    ...
    def propose(
        ...
        if decision.action is not LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationAction.ACCEPT:
            return None
        ...
        return LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposal(
            ...
            proposal_source_id=decision.proposal_source_id,
            domain=decision.domain,
            source_policy_id=decision.source_policy_id,
            policy_id=decision.policy_id,
            kind=LearningWriteAdaptationEvaluationExecutionFeedbackResultIntegrityFeedbackEvaluationDecisionProposalKind.ACCEPTED_EVALUATION,
            action=decision.action,
            confidence=float(decision.confidence),
            reason="accepted evaluation decision converted into a downstream proposal",
            payload=payload,
            evidence=evidence_data,
            provenance=provenance_data,
            metadata={"source": "m22.46", "proposal_authority": False},
        )

    @staticmethod
    def _proposal_id(decision: Any, payload: Mapping[str, Any], evidence: Mapping[str, Any]) -> str:
        serialized = json.dumps(
            {"decision_id": decision.decision_id, "evaluation_id": decision.evaluation_id, "feedback_id": decision.feedback_id, "execution_id": decision.execution_id, "payload": payload, "evidence": evidence},
            sort_keys=True,
            default=repr,
            separators=(",", ":"),
        ).encode("utf-8")
        return "adaptation-evaluation-execution-feedback-result-integrity-proposal-" + hashlib.sha256(serialized).hexdigest()[:24]
