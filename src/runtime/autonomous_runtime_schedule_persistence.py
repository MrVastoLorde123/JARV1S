...
from src.runtime.autonomous_runtime_scheduler import AutonomousRuntimeSchedule, AutonomousRuntimeScheduleStore, scheduler_claim_token


class SQLiteAutonomousRuntimeScheduleStore(AutonomousRuntimeScheduleStore):
    """Durable SQLite-backed scheduler store with atomic claim fencing."""
    ...
    def claim(
        ...
        try:
            ...
            if active_token is not None and active_lease_until > now:
                connection.rollback()
                return None
            connection.execute(
                """
        ...
        try:
            ...
            if replacement is None:
                cursor = connection.execute(
                    """
                    DELETE FROM autonomous_runtime_schedules
