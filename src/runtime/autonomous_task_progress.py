...
class AutonomousTaskProgressResult:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.step_count_before, bool) or not isinstance(self.step_count_before, int) or self.step_count_before < 0:
            raise ValueError("step_count_before must be a non-negative integer")
        if isinstance(self.step_count_after, bool) or not isinstance(self.step_count_after, int) or self.step_count_after < 0:
            raise ValueError("step_count_after must be a non-negative integer")
...
class DeterministicAutonomousTaskProgressEvaluator:
    ...
    def evaluate(
        ...
        if after.status is AutonomousJobStatus.COMPLETED:
            return AutonomousTaskProgressResult(
                AutonomousTaskProgressVerdict.COMPLETED,
                "task reached its runtime completion state",
                after.result or "completion result persisted",
                before_count,
                after_count,
            )
        if after.status in {
            ...
            return AutonomousTaskProgressResult(
                AutonomousTaskProgressVerdict.BLOCKED,
                "task is waiting for an external condition",
                after.waiting_reason or "waiting state persisted",
                before_count,
                after_count,
            )
        if after.status in {AutonomousJobStatus.FAILED, AutonomousJobStatus.CANCELLED}:
            return AutonomousTaskProgressResult(
                AutonomousTaskProgressVerdict.FAILED,
                "task entered a terminal non-success state",
                after.failure_reason or after.status.value,
                before_count,
                after_count,
            )
        if after_count <= before_count:
            return AutonomousTaskProgressResult(
                AutonomousTaskProgressVerdict.NO_PROGRESS,
                "bounded cycle produced no durable work-step advance",
                cycle.summary,
                before_count,
                after_count,
            )
        if cycle.disposition is AutonomousCycleDisposition.CONTINUE:
            return AutonomousTaskProgressResult(
                AutonomousTaskProgressVerdict.PROGRESSED,
                "bounded cycle recorded durable work progress",
                cycle.observation or cycle.summary,
                before_count,
                after_count,
            )
        return AutonomousTaskProgressResult(
            AutonomousTaskProgressVerdict.PROGRESSED,
            "bounded cycle changed durable task state",
            cycle.observation or cycle.summary,
            before_count,
            after_count,
        )
