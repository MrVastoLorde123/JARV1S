...
class AutonomousReasoningWorker:
    ...
    def run_cycle(self, job: AutonomousJob) -> AutonomousCycleResult:
        ...
        if d is AutonomousReasoningDisposition.FAIL:
            return AutonomousCycleResult(
                AutonomousCycleDisposition.FAIL,
                "reasoning",
                action.rationale,
                reason=action.wait_reason or "reasoning failed",
                context_delta=base,
            )
        ...
        return AutonomousCycleResult(
            AutonomousCycleDisposition.FAIL,
            "reasoning",
            "unsupported reasoning disposition",
            reason=f"unsupported reasoning disposition: {d!r}",
        )
