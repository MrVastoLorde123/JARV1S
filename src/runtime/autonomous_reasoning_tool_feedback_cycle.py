...
from src.runtime.autonomous_tool_authorization import matches_tool_resume_authorization
...
class AutonomousReasoningToolFeedbackCycleCoordinator:
    def __init__(self, worker: AutonomousReasoningWorker, tool_gate: AutonomousReasoningToolGate, feedback=None):
        if not isinstance(worker, AutonomousReasoningWorker):
            raise TypeError("worker must be an AutonomousReasoningWorker")
        if not isinstance(tool_gate, AutonomousReasoningToolGate):
            raise TypeError("tool_gate must be an AutonomousReasoningToolGate")
        self._worker = worker
        self._tool_gate = tool_gate
        self._feedback = feedback or AutonomousToolResultFeedbackAdapter()
    ...
    def run_cycle(
        ...
        *,
        ...
        if authorization_token is not None and not isinstance(authorization_token, Mapping):
            raise TypeError("authorization_token must be a mapping or None")
        ...
        request_confirmed = confirmed or matches_tool_resume_authorization(
            authorization_token,
            action.tool_name or "",
            action.arguments,
        )
        gate = self._tool_gate.evaluate(action, confirmed=request_confirmed)
        if gate.authorization_required:
            cycle = AutonomousCycleResult(
                AutonomousCycleDisposition.WAIT_TOOL,
                "tool_authorization",
                action.rationale,
                reason=gate.reason or "tool authorization required",
                context_delta={
                    "reasoning_action": action.to_dict(),
                    "pending_tool_request": {
                        "tool_name": gate.tool_request.tool_name,
                        "arguments": dict(gate.tool_request.arguments),
                        "invocation_id": gate.tool_request.invocation_id,
                    },
                },
            )
        ...
        if gate.tool_result is None:
            cycle = AutonomousCycleResult(
                ...
                "tool_execution",
                "tool gate returned no result after execution",
                reason="tool execution produced no ToolResult",
                context_delta={"reasoning_action": action.to_dict()},
            )
        ...
        cycle = AutonomousCycleResult(
            ...
            "tool_feedback",
            f"Tool '{feedback.tool_name}' executed; result is available for the next reasoning cycle.",
            observation=feedback.observation,
            context_delta={"reasoning_action": action.to_dict(), **feedback.context_delta},
        )
        return AutonomousReasoningToolFeedbackCycle(cycle, action, gate.executed, False)
