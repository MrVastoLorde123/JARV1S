"""Durable SQLite persistence for immutable autonomous-job snapshots."""
...
class SQLiteAutonomousJobStore(AutonomousJobStore):
    """Durable SQLite-backed store for complete autonomous-job snapshots."""
    ...
    def _from_dict(snapshot: object) -> AutonomousJob:
        if not isinstance(snapshot, dict):
            raise AutonomousJobValidationError(
                "stored autonomous-job snapshot must be an object"
            )
        ...
        if not isinstance(raw_steps, list) or not isinstance(raw_events, list):
            raise AutonomousJobValidationError(
                "stored autonomous-job steps/events must be arrays"
            )
        ...
        if len(steps) != len(raw_steps):
            raise AutonomousJobValidationError(
                "stored autonomous-job steps contain an invalid entry"
            )
        ...
        if len(events) != len(raw_events):
            raise AutonomousJobValidationError(
                "stored autonomous-job events contain an invalid entry"
            )
