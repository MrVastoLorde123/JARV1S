...
function guidance(snapshot: JarvisSnapshot) {
  ...
  if (snapshot.attentionRequired || snapshot.mode === 'Needs You') {
    return { kind: 'ATTENTION', label: 'I need you here', message: snapshot.attentionReason || 'A decision is waiting for you.', target: 'ATTENTION' };
  }
  ...
  if (snapshot.mode === 'Thinking') {
    return { kind: 'THINKING', label: 'I am thinking here', message: snapshot.currentFocus, target: 'CONTEXT' };
  }
  if (snapshot.mode === 'Learning') {
    return { kind: 'LEARNING', label: 'I am learning here', message: snapshot.researchTopics[0]?.title ?? 'I am connecting what I have learned.', target: 'CONTEXT' };
  }
  if (snapshot.mode === 'Monitoring') {
    return { kind: 'MONITORING', label: 'I am watching here', message: `${snapshot.resources.pressure} pressure · ${snapshot.monitoredSources} monitored sources`, target: 'RUNTIME' };
  }
