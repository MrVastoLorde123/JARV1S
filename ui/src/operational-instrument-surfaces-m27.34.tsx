...
export function OperationalInstrumentSurface({ space, snapshot }: Props) {
  ...
  if (space === 'CHAT') {
    ...
    return <InstrumentShell space={space} eyebrow="INTENT MEMBRANE" title="Intent enters here." subtitle="CHAT is the intake instrument: language, direction and outcomes enter the machine before JARVIS decides what kind of work they become." cards={cards} mode="intent" signal={signal} stateLabel={stateLabel} signals={signals}>
  ...
  if (space === 'CONTROL') {
    ...
    return <InstrumentShell space={space} eyebrow="ACTION CONSOLE" title="The machine acts here." subtitle="CONTROL is the action instrument: selected mission, authority boundaries, model assignment and guarded transitions converge here." cards={cards} mode="control" signal={signal} stateLabel={stateLabel} signals={signals}>
  ...
  if (space === 'MIND') {
    const cards = [
      ...
      metric('CONTEXT DEPTH', signals.contextDepth, `${snapshot.projects.length} work + ${snapshot.researchTopics.length} research + ${snapshot.plans.length} plans + ${snapshot.selfActivity.length} recent runtime signals.`),
    ...
    return <InstrumentShell space={space} eyebrow="CONTEXT FIELD" title="The machine carries context here." subtitle="MIND is the internal-context instrument: memory, knowledge, plans, experiences and relationships give future actions somewhere to come from." cards={cards} mode="mind" signal={signals.contextDepth >= 16 ? 'CONTEXT-RICH' : signal} stateLabel={signals.contextDepth >= 16 ? 'CONTEXT RICH' : stateLabel} signals={signals}>
  ...
  const cards = [
    metric('AVAILABLE', readyCount, unavailableCount ? `${unavailableCount} capability records are currently unavailable.` : `${snapshot.capabilities.length} capability records are currently represented.`),
...
function InstrumentShell({ space, eyebrow, title, subtitle, cards, mode, signal, stateLabel, signals, children }: { space: Space; eyebrow: string; title: string; subtitle: string; cards: Array<{ label: string; value: string; detail: string }>; mode: string; signal: RuntimeSignal; stateLabel: string; signals: ReturnType<typeof deriveSignals>; children: React.ReactNode }) {
...
export function installOperationalInstrumentSurfaces() {
  ...
  const sync = () => {
    ...
    if (!mount || currentSpace !== next || !container.contains(mount)) {
      root?.unmount(); mount?.remove(); mount = document.createElement('div'); mount.className = 'operational-instrument-mount'; container.insertBefore(mount, container.firstChild); root = createRoot(mount); currentSpace = next;
    }
